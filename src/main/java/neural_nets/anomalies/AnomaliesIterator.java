...
public class AnomaliesIterator extends CharacterIterator {
    ...
    public AnomaliesIterator(String textFilePath, String testFilePath, Charset encoding, int miniBatchSize, int sequenceLength, int epochSize,
                             boolean minimized) throws IOException {
            if(!new File(textFilePath).exists()) throw new IOException("Could not access textFilePath (does not exist): " + textFilePath);
